...
def _gather_legs(
    ...
    request_targets: Dict[int, float],
) -> List[_LegRender]:
    out: List[_LegRender] = []
...
def _extract_request_targets(episode_log: Sequence[dict]) -> Dict[int, float]:
    ...
    if summary is None:
        return {}
    return {
        int(r["req_id"]): float(r["t_delivery_target"])
...
def _extract_pickup_targets(episode_log: Sequence[dict]) -> Dict[int, float]:
    ...
    if summary is None:
        return {}
    return {
        int(r["req_id"]): float(r["t_pickup_target"])
        for r in summary.get("request_history", [])
    }

def _extract_arrival_times(
    episode_log: Sequence[dict],
    ...
    *,
    ...
    if show_arrivals and arrival_times:
        ax.scatter(
            ...
            marker="v", s=18, color=COLOR_ARRIVAL, alpha=0.65, zorder=2,
        )
...
if __name__ == "__main__":
    ...
    fleet = {
        0: _DummyVeh("uav"), 1: _DummyVeh("uav"),
    ...
    arrivals = [{"t_arrival": 1.0}, {"t_arrival": 5.5}, {"t_arrival": 8.0}]

    fig, ax = plt.subplots(figsize=(12, 4))
    plot_shift_timeline(log, fleet, ax=ax, arrival_stream=arrivals,
                        title="Smoke test")
    fig.savefig("shift_timeline_smoke.png", bbox_inches="tight", dpi=120)
    print("Wrote shift_timeline_smoke.png")
